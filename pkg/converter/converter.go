...
import (
	...
	"github.com/willdavsmith/bicep-types-go/factory"
	"github.com/willdavsmith/bicep-types-go/index"
	"github.com/willdavsmith/bicep-types-go/types"
	"github.com/willdavsmith/bicep-types-go/writers"
	"github.com/willdavsmith/bicep-tools-go/pkg/manifest"
)
...
func Convert(provider *manifest.ResourceProvider) (*ConversionResult, error) {
	...
	normalizedName := strings.ReplaceAll(strings.ToLower(provider.Name), ".", "")
...
func addResourceTypeForAPIVersion(
	...
	qualifiedName := fmt.Sprintf("%s/%s@%s", provider.Name, resourceTypeName, apiVersionName)
	...
	bodyType.Properties = map[string]types.ObjectTypeProperty{
		...
		"type": {
			Type:        typeFactory.GetReference(typeFactory.CreateStringLiteralType(fmt.Sprintf("%s/%s", provider.Name, resourceTypeName))),
			Flags:       types.TypePropertyFlagsReadOnly | types.TypePropertyFlagsDeployTime | types.TypePropertyFlagsConstant,
	...
	resourceTypeRef := typeFactory.CreateResourceType(
		qualifiedName,
		fmt.Sprintf("%s/%s", provider.Name, resourceTypeName),
		apiVersionName,
